...
class ABUpdateInstaller {
    ...
    private final UpdateEngineCallback mUpdateEngineCallback = new UpdateEngineCallback() {
        ...
        public void onStatusUpdate(int status, float percent) {
            ...
            if (update == null) {
                // We read the id from a preference, the updater could no longer exist
            ...
            switch (status) {
                ...
                case UpdateEngine.UpdateStatusConstants.IDLE: {
                    ...
                    // updater, but we aren't, so clear everything.
    ...
    void install(String downloadId) {
        if (isInstallingUpdate(mContext)) {
            Log.e(TAG, "Already installing an updater");
            return;
        }
        ...
        if (!file.exists()) {
            Log.e(TAG, "The given updater doesn't exist");
    ...
    void reconnect() {
        if (!isInstallingUpdate(mContext)) {
            Log.e(TAG, "reconnect: Not installing any updater");
            return;
        }
    ...
    public boolean cancel() {
        if (!isInstallingUpdate(mContext)) {
            Log.e(TAG, "cancel: Not installing any updater");
            return false;
        }

        if (!mBound) {
            Log.e(TAG, "Not connected to updater engine");
            return false;
        }
    ...
    public boolean resume() {
        if (!isInstallingUpdateSuspended(mContext)) {
            Log.e(TAG, "cancel: No updater is suspended");
            return false;
        }
